...
from django_cassandra_fake.base.client import CassandraDatabaseClient
from django_cassandra_fake.base.creation import CassandraDatabaseCreation
from django_cassandra_fake.base.schema import CassandraDatabaseSchemaEditor
from django_cassandra_fake.base.features import CassandraDatabaseFeatures
from django_cassandra_fake.base.operations import CassandraDatabaseOperations
from django_cassandra_fake.base.validation import CassandraDatabaseValidation
from django_cassandra_fake.base.introspection import (
    CassandraDatabaseIntrospection,
)
from django_cassandra_fake.utils import CursorWrapper
...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def get_new_connection(self, conn_params):
        from django_cassandra_fake.connection import (
            FakeConnection
        )
    ...
    def connect(self):
        from django_cassandra_fake.connection import (
            InMemoryCassandraConnection
        )
        with self._connect_lock:
            if not self.connected or self.connection is None:
                settings = self.settings_dict
                self.connection = InMemoryCassandraConnection(self.alias, **settings)
